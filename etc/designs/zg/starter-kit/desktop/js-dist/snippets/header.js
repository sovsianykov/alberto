(function($) {
	...
	var SELECTORS = {
		...
		referenceHeader: "[class*=reference-header]",
		mobileSupplementaryHeader: ".nav-mobile .header-supplementary",
		desktopSupplementaryHeader: "[class*=reference-header] .header-supplementary",
		tabContentNavItemTitle: ".tabContent .navigation-item .navigation-item-title",
		navigationItem: ".navigation-item",
		hasChildren: "has-children",
		directComponentContent: ">.component-content"
	};
	...
	function linkClickAnalytics(href, label, componentPosition) {
		Cog.fireEvent("link", analyticsEvents.LINK_CLICK, {
			href: href,
			label: label,
			category: ctConstants.engagement,
			subcategory: ctConstants.read,
			componentName: ctConstants.navigation,
			type: ctConstants.navigation,
			componentPosition: componentPosition
		});
	}

	function cloneSupplementaryHeaderWithEvents() {
		$(SELECTORS.mobileSupplementaryHeader).find(SELECTORS.directComponentContent).remove();
		var headerSuplementary = $(SELECTORS.desktopSupplementaryHeader).first().find(SELECTORS.directComponentContent).clone(true);
		$(SELECTORS.mobileSupplementaryHeader).append(headerSuplementary);
		Cog.fireEventAsync("Navigation", "updateMobileMenuHeight", {
			$elem: $(SELECTORS.mobileSupplementaryHeader)
		});
	}
	...
	MobileNav.prototype = {
		buildHTML: function() {
			...
			$tabs.find(SELECTORS.tabContentNavItemTitle).on("click", function(event) {
				var itemHasChildLinks = $(this).closest(SELECTORS.navigationItem).hasClass(SELECTORS.hasChildren);
				var self = $(event.target);
				if (!itemHasChildLinks) {
					linkClickAnalytics(self.attr(ATTRIBUTES.href), self.attr(ATTRIBUTES.title) || self.text(),
						self.closest(SELECTORS.navigation).attr(ATTRIBUTES.position));
				}
			});
		...
		isStandaloneLink: function($holder) {
			...
			if ($childLinks.length === 1) {
				linkClickAnalytics($childLinks.attr(ATTRIBUTES.href), $childLinks.attr(ATTRIBUTES.title) || $childLinks.text(),
					$childLinks.closest(SELECTORS.navigation).attr(ATTRIBUTES.position));
		...
		bindEvents: function() {
			...
			Cog.addListener("Navigation", "updateMobileMenuHeight", function(event) {
				this.refreshScroll(event.eventData.$elem);
			}.bind(this));
		}
	};
	...
	DesktopNavItem.prototype = {
		...
		go: function() {
			if (this.isStandaloneLink) {
				linkClickAnalytics(this.$childLinks.attr(ATTRIBUTES.href), this.$childLinks.attr(ATTRIBUTES.title) || this.$childLinks.text(),
					this.$childLinks.closest(SELECTORS.navigation).attr(ATTRIBUTES.position));
	...
	var api = {
		onRegister: function(scope) {
			...
			analyticsEvents = this.external.eventsDefinition.CLICK;
			...
			Cog.addListener("Navigation", "MobileMenuAnalytics", function() {
				setTimeout(cloneSupplementaryHeaderWithEvents, 0);
			});
		}
	};
