...
(function($) {
	...
	Listing.prototype = {
		...
		trackImpression: function() {
			if (this.$el.find(".show-more").length) {
				this.trackImpressionOnLoad(this.$items.filter(":visible").not(CLASSES.promotileItem));
			} else if (this.$el.hasClass("listing--as-carousel")) {
				this.trackImpressionOnLoad(this.$items.filter(this.isProductVisible).not(CLASSES.promotileItem));
				...
				this.trackImpressionOnLoad(this.$items.not(CLASSES.promotileItem));
			}
		},
				...
				this.carouselImpressionInProgress = setTimeout(function() {
					if (this.listingType === analyticsDef.ctConstants.product) {
						Cog.fireEvent("listing",
							...
							{
								products: this.$items.filter(this.isProductVisible).not(CLASSES.promotileItem),
	...
	api.onRegister = function(scope) {
		...
		addArticleClickListener($items.not(CLASSES.promotileItem));
