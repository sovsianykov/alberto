(function($) {
	...
	window.googleStoreCallback = function() {
		Cog.fireEvent("stores.map", "ready");
	};
	...
	GMaps.prototype = {
		...
		bindEvents: function() {
			Cog.addListener("stores.map", "ready", this.onLoadScript);
			Cog.addListener("stores.map", "reset", this.clearMarkers);
			Cog.addListener("stores.map", "userlocation.change", this.panTo);
			Cog.addListener("stores.map", "userlocation.change", this.sessionPos);
		...
		loadScript: function() {
			...
			script.src = this.config.mapsUrl
				...
				.replace("{callback}", "googleStoreCallback");
