(function($) {
	...
	rules.add("[data-validate-checkbox-length]", function($holder) {
		var $maxCheckboxCount = $holder.attr("data-max-checkbox-value");
		if ($maxCheckboxCount === 0 || $maxCheckboxCount === "") {
			$maxCheckboxCount = $holder.find("[type=checkbox]").length;
		}
		var valid = (function() {
			var checkboxLenght = $holder.find("[type=checkbox]:checked").length;
			if (checkboxLenght > $maxCheckboxCount) {
				return false;
			}
			return true;
		})();

		return (valid || {msg: errorMsg($holder, "excess-checkbox-selected-error-message")});
	});

	rules.add("[required],.g-recaptcha", function($holder) {
		var $field = $holder.find("input, select, textarea");
		var valid = (function() {
			if ($holder.attr("data-validate-checkbox-length")) {
				var minCheckboCount = $holder.attr("data-min-checkbox-value");
				var checkboxCheckedCount = $holder.find("[type=checkbox]:checked").length;
				if (minCheckboCount > checkboxCheckedCount) {
					return false;
				} else {
					return true;
				}
